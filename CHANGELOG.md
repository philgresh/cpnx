...
- **Dependency-health circuit breaker — stop firing the transitions that depend on a downed external service, hold their tokens in place, and resume automatically on recovery.** Two new primitives, expressed as net state rather than an application global (which was previously the only option: a module-level "is-it-up?" flag plus a lock, invisible to `validate`/`snapshot`/`to_dot`, racy, and not reusable across a second dependency). See [`docs/adr/0007-dependency-health-circuit-breaker.md`](docs/adr/0007-dependency-health-circuit-breaker.md), [`examples/circuit_breaker.py`](examples/circuit_breaker.py), [`tests/test_test_arc.py`](tests/test_test_arc.py), and [`tests/test_circuit_breaker.py`](tests/test_circuit_breaker.py).

  - **Non-consuming test/read arc — `InputArc(..., test=True)`.** Gates a transition on token *presence* (at least `count` available) without consuming anything: it binds no specific token, resolves to an empty consume set in the binding, is skipped by `_consume_binding` (so its tokens never enter the rollback/retry path), and is never passed to the transition `guard`. Because it binds no token id, **many transitions can test the same place concurrently** without serializing — the property a shared gate needs, and the one a consuming `InputArc` (or a `ResourcePlace` permit borrowed-and-returned) cannot provide. Enablement reuses the existing `Place.can_retrieve(count)` check, so no new satisfiability path is introduced. `test=True` with `consume_all=True` is a contradiction and raises `TypeError`; `key`/`filter` are ignored on a test arc (there is nothing to order or select, only to count).

  - **`CircuitBreakerPlace`.** A self-contained `Place` owning a binary health signal plus its lifecycle state (`closed`/`open`), consecutive-failure count, cooldown deadline, and optional probe. Dependent transitions — both the failing downstream one *and* the expensive upstream ones — gate on it via a test arc; a transition whose failures should trip it names it via the new `Transition.breaker` field (**multiple transitions may share one breaker**, since the dependency is shared). The engine feeds each bound transition's outcome to the breaker under the engine lock right after commit/rollback: a consumer-supplied `trip_predicate(exc) -> bool` classifies failures, and after `failure_threshold` *consecutive classified* failures the breaker opens for `cooldown_secs`; a success resets the count. While open, `can_retrieve` returns `False` for a real firing, so every gated transition is disabled and its input tokens queue in place — no busy-spin, no wasted upstream compute. Recovery is serviced once per `run`/`drive_to_quiescence` loop pass: once the cooldown elapses, a supplied `probe()` runs **off the engine lock** (it may do I/O), exactly one at a time — a truthy result closes the breaker, a falsy result or an exception re-opens it; with no probe, the breaker closes optimistically and the next real firing is the trial. Because a gated transition never fires while open, at most one probe is ever in flight. `trip()`/`close()` allow out-of-band control (learning of the outage or recovery from outside the net).

  - **Reuses the engine's timed-token vocabulary rather than adding scheduler machinery.** An open breaker reports as *eventually available* under the `+inf` ignore-timing sentinel, so `is_quiescent` treats a net with work queued behind an open breaker as "waiting for a boundary" (not quiescent) while `is_dead` still reports it dead for the current instant; `earliest_available_boundary` returns the cooldown deadline so `run()` wakes to service the probe. Registering a `CircuitBreakerPlace` sets `_has_timed_features`, so the net runs on the full-scan scheduler — correct, because the cooldown re-arm is clock-driven with no marking mutation, exactly the class of re-enablement [`docs/adr/0006-incremental-enablement.md`](docs/adr/0006-incremental-enablement.md) excludes from the dirty-set fast path.

  - **Analyzable.** `validate()` enforces that a breaker place is referenced only by test arcs (a consuming input arc, or an output arc targeting a breaker, raises `TypeError`) and that a `breaker` binding names a registered `CircuitBreakerPlace`. `snapshot` surfaces the breaker's `state`/`consecutive_failures`/`probe_at`/`probing`, and `to_dot` draws it as a double circle labelled with its state, with test arcs rendered as dashed, hollow-headed edges. `CircuitBreakerPlace` is exported from the package root.
