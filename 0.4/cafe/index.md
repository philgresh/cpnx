...
A second colour-restricted Place (`{"milk_ticket"}`), and — jointly with p_ground_coffee — the net's **fork**: one transition writing two output arcs into two different places, so the two downstream stations become independently enabled and can genuinely run concurrently.
...
The tower itself — one colour-restricted Place holding steeping batches.
...
The backlog — an unbounded FIFO Place, same shape as `P_Ticket_Line`.
